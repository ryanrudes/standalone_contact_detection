"""Regression + richer-output suite for the UPGRADED detector (THEORY.md §5-§7).

This sibling of ``test_integration.py`` guards two things at once on the *same*
end-to-end pipeline of THEORY.md §9 ---
...
withheld simulator truth:
  ...
  (A) **No regression.** The capabilities the lower rungs already had must survive the
      §5-§7 upgrade stack (gap-gated transitions, semi-Markov decoding, impact atoms,
  ...
  (B) **The richer outputs now exist.** The upgraded ``DetectionResult`` populates the
      *new* fields the contracts added (types.py): ``impulses`` (force-as-measure atoms,
      §6) become non-empty on a bouncer, and with a material ``stiffness`` set the
      penetration-as-force gauge populates ``normal_force`` (>0 on contact frames, §7)
      and the friction layer populates ``slip_state`` (§7).
...
# Skip cleanly without the simulator (THEORY.md §9: it is the truth oracle, but the
...
def _run(name: str, config: DetectorConfig | None = None):
    """Run the full §9 pipeline for one scenario: (raw, obs, result, scores).

    generate (clean physics + withheld truth, noisy poses) -> observe (support-relative
    twist, §1/§3) -> detect (the HMM estimator with the §5-§7 upgrades) -> score.
    The optional ``config`` lets a test switch on the material model (§7).
    """
...
def _map_runs(result, label: str) -> list[tuple[float, float]]:
    ...
    THEORY.md §5: the Viterbi/HSMM MAP path is temporally coherent. We read per-frame
...
def _frac_mode_on_true_contact(result, truth, label: str) -> float:
    """Fraction of *truly-in-contact* frames whose MAP mode equals ``label`` (§3)."""
...
def test_drop_rest_existence_not_regressed():
    """drop_rest: existence IoU stays high after the §5-§7 upgrades.

    Spec bound 0.70; the upgraded detector measures ~0.96 at this seed. The shortfall
    from 1.0 is the few-frame onset lag inherent to a smoothing detector (§6: a landing
...
def test_moving_support_relative_frame_payoff_retained():
    """moving_support: the §1 relative-frame payoff survives the upgrade.
...
def test_rolling_ball_still_detected_as_rolling():
    """rolling_ball: ROLLING is still recovered after the upgrade (THEORY.md §3).
    ...
    assert roll_frac > 0.30, (
        ...
        f"the curved-manifold rolling mode of §3 regressed"
    )
...
def test_push_to_slide_has_sliding_interval():
    """push_to_slide: a SLIDING segment is still recovered (THEORY.md §7 stick->slip).
...
# --------------------------------------------------------------------------------------
# (A) Events on drop_rest_liftoff still pinned (the two guards of THEORY.md §5)
# --------------------------------------------------------------------------------------


def test_drop_rest_liftoff_events_still_found():
    """drop_rest_liftoff: touchdown AND liftoff events still found (THEORY.md §5).
    ...
    assert len(touchdowns) >= 1, "expected a touchdown (free->contact guard, §5)"
    assert len(liftoffs) >= 1, "expected a liftoff (contact->free guard, §5)"

    # Causality: you cannot break a contact you never made (guard ordering, §5/§6).
...
# --------------------------------------------------------------------------------------
# (B) Richer outputs: impact atoms (THEORY.md §6)
# --------------------------------------------------------------------------------------


def test_bouncing_ball_impulses_non_empty():
    """bouncing_ball: ``result.impulses`` is non-empty (THEORY.md §6).
    ...
    populated; an empty list would mean the §6 impact layer is not wired in.
    """
    _raw, _obs, result, _scores = _run("bouncing_ball")

    assert len(result.impulses) >= 1, (
        "bouncing_ball produced no impact atoms; the §6 force-as-measure layer "
...
# --------------------------------------------------------------------------------------
# (B) Richer outputs: penetration-as-force gauge + friction state (THEORY.md §7)
# --------------------------------------------------------------------------------------


def test_material_stiffness_populates_force_and_slip_state():
    """With material.stiffness set, normal_force and slip_state are populated (§7).

    THEORY.md §7: known compliance turns the penetration depth into a calibrated force
    ...
    non-negative everywhere (Signorini: a contact only pushes, §2), with a strictly
    ...
    # --- (a) normal_force is populated and physically sane (§2 / §7) ---
    assert result.normal_force is not None, (
        "stiffness set but normal_force stayed None; the §7 force gauge is not wired"
    )
    ...
    # Signorini: a contact only pushes, never pulls -> force >= 0 wherever finite (§2).
    assert np.all(finite >= -1e-9), (
        f"normal_force has a negative (pulling) value min={finite.min():.4f} N (§2)"
    )
    ...
    # Strictly positive peak on contact frames -- the gauge actually reads a load (§7).
    assert float(np.max(nf_on)) > 0.0, (
        "no contact frame carries a positive normal force; the penetration-as-force "
        "gauge (lambda = k*delta) is not reading any load (§7)"
    )
    ...
    assert loaded_frac > 0.20, (
        f"only {loaded_frac:.2%} of contact frames carry positive force; the §7 gauge "
        f"is barely engaging"
    )

    # --- (b) slip_state is populated (§7 friction layer) ---
    assert result.slip_state is not None, (
        "stiffness set but slip_state stayed None; the §7 friction layer is not wired"
    )
    slip = list(result.slip_state)
    assert len(slip) == result.t.shape[0], "slip_state must be per-frame (length T)"
    # Off-contact frames read "" (no friction state without a contact, §2); contact
    ...
    # Off-contact frames must be unlabeled (§2: no friction state without a contact).
