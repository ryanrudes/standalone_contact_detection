...
This is the test suite for the FINAL rung of THEORY.md §10 -- the §8 "north star":
...
``contact.inverse_dynamics`` (``contact_implicit_from_raw`` end-to-end); the truth oracle is
``contact.oracle.factory`` (THEORY.md §9: simulate -> withhold the truth from the detector
...
double-differentiating those noisy poses (THEORY.md §4/§6: differentiation amplifies
...
* **drop_rest -- weight recovery at rest.** Over the SETTLED window the recovered
  ...
  rest is exactly the four box-bottom corners, EMPTY during free flight (Signorini, §2),
...
* **drop_rest -- force tracking over the contact phase.** The recovered total force must
  ...
  *cannot* reproduce MuJoCo's 2000 N single-frame touchdown spike -- §6: differentiated
  ...
  bandwidth, which is the only fair comparison (THEORY.md §4/§6).
* **indeterminate_rig -- weight + a valid, regularized split.** The recovered total is
  ...
  is the **regularized minimum-norm** choice among the §7 indeterminate family -- it is
  the regularizer's pick, NOT a measurement (THEORY.md §7/§8); only per-corner
...
from contact.inverse_dynamics import contact_implicit_from_raw
...
def _rest_window(scenario, frac_start: float = 0.7) -> slice:
    ...
    force should equal the held-up weight ``m*g`` (THEORY.md §3 STATIC, §8).
    """
...
# --------------------------------------------------------------------------------------
# drop_rest: weight recovery, active set, residual (THEORY.md §8 + §2)
# --------------------------------------------------------------------------------------
...
def test_drop_rest_recovers_weight_at_rest():
    ...
    (THEORY.md §4) and MuJoCo's contact is a damped soft constraint that leaves a low
    residual ringing in the settled accel (THEORY.md §9). Both perturb the recovered
    ...
    assert recovered == pytest.approx(mg, rel=0.15), (
        ...
        f"damped-solver residual; THEORY.md §4/§9)"
    )


def test_drop_rest_active_set_is_all_corners_at_rest_and_empty_in_flight():
    """Active set = all four bottom corners at rest; EMPTY during free flight (§2).

    Signorini (THEORY.md §2): a candidate may carry force only where its gap is closed.
    ...
    assert K == 4, "drop_rest exposes the four box-bottom corners as candidates (§8)"
    ...
    for i in flight_pre:
        assert res.active_set[i] == [], (
            ...
            "Signorini forbids force where the gap is open (THEORY.md §2)"
        )
    ...
    for i in range(rest.start, rest.stop):
        assert sorted(res.active_set[i]) == all_corners, (
            ...
            f"loads all {K} bottom corners (THEORY.md §8)"
        )
...
def test_drop_rest_wrench_residual_small_at_rest():
    ...
    assert mean_residual < 0.05 * mg, (
        ...
        f"{mg:.2f} N (the four corner forces span the weight wrench exactly; THEORY.md §8)"
    )
...
def test_drop_rest_tracks_true_summed_corner_force_over_contact():
    ...
    single-frame ~2000 N touchdown atom (THEORY.md §6). The recovery is band-limited to
    the estimator's ``accel_smooth_time`` window and *cannot* reproduce that atom -- §4/§6
    ...
    assert corr > 0.8, (
        ...
        f"0.8 over the contact phase (THEORY.md §6: the recovery cannot resolve the "
...
# --------------------------------------------------------------------------------------
# indeterminate_rig: weight + a valid regularized minimum-norm split (THEORY.md §7/§8)
# --------------------------------------------------------------------------------------
...
def test_indeterminate_rig_recovers_weight_and_valid_split():
    ...
    THEORY.md §7 (the deepest result): a box on four corners is statically indeterminate
    -- an entire family of corner-force splits produces the *identical* net wrench, so the
    split is UNRECOVERABLE from kinematics alone. ``inverse_dynamics`` does not pretend that
    ...
    choice, not a measurement (THEORY.md §7/§8). Only per-corner compliance
    ...
    ||f_t|| <= mu*f_n, §7). We deliberately do NOT assert the split equals the true split,
    ...
    assert recovered_total == pytest.approx(mg, rel=0.15), (
        ...
        f"THEORY.md §7); double-diff + damped-solver band as in drop_rest"
    )
    ...
    # Non-negativity (Signorini: contact can only push, §2). Allow a tiny numerical slack.
    assert fn.min() >= -1e-6, (
        f"per-corner normal forces must be >= 0 (contact only pushes, THEORY.md §2); "
        f"min was {fn.min():.3e} N"
    )

    # Coulomb friction cone per corner (THEORY.md §7): ||f_t|| <= mu*f_n. A small absolute
    ...
    assert cone_violation.max() <= 1e-6, (
        ...
        f"||f_t|| <= mu*f_n (mu={mu}, THEORY.md §7); max violation "
    ...
    # is the regularizer's indeterminate choice (THEORY.md §7), so on a few frames it sheds
    ...
    for i in range(rest.start, rest.stop):
        ...
        assert active <= corners, (
            ...
            f"candidate (only {K} corners exist; THEORY.md §8)"
        )
    ...
    assert mean_active >= 3.5, (
        ...
        f"majority of frames (THEORY.md §7/§8; it sheds the lightest corner only "
