"""Multi-body contact-graph detector — the active-set structure posterior (THEORY.md §8).

This is rung 5 of the pragmatic ladder (THEORY.md §10): the single-pair estimator of
:class:`contact.detector.ContactDetector` is lifted from one body-pair to a whole *contact
...
time, as a Bayesian posterior (THEORY.md §8, first paragraph).

The object assembled here is exactly the §8 sentence, restricted to the existence
...
:class:`~contact.types.DetectionResult`):
  ...
  * a **Bayesian posterior over active-constraint structures** — the joint active set —
    obtained by running the §5 HMM machinery over the alphabet of the ``2**E`` subsets
    of edges (:func:`detect_scene`);
  * the **Signorini complementarity** of §2 as the legality prior (each subset is a
    legal active set; per-edge Signorini is already enforced inside each edge's HMM);
  * the **hybrid temporal prior** of §5 (the active set is temporally coherent — it
  ...
  * optional **energy/dissipation** and **balance** consistency factors that couple the
    edges globally (THEORY.md §8: "an energy/dissipation budget ... is a global
    consistency check linking all contacts"), supplied by :mod:`contact.consistency`.

Tractability (THEORY.md §8). For ``E`` edges there are ``2**E`` candidate active sets.
...
and one would instead use RJMCMC / particle methods over the structure (THEORY.md §8,
...
:func:`contact.geometry.observe`, which is the §1 relative-frame payoff: the same
...
reuses :class:`contact.detector.ContactDetector` per edge, :func:`contact.geometry.observe`
...
# Conventional name of the implicit, infinite-mass static support (THEORY.md §1). A
...
# degenerate §1 support that never moves. We synthesize an identity PoseTrajectory for it
...
def _resolve_support(
    ...
    THEORY.md §1: the static floor is the special case of an infinite-mass support whose
...
# reason and inject NaNs into forward-backward). THEORY.md §4: posteriors are soft.
_PROB_EPS = 1e-6


# --------------------------------------------------------------------------------------
# Broad phase (THEORY.md §8: "proximity used as a broad-phase filter so we don't test
...
def build_candidate_edges(
    ...
    """Broad-phase prune of the scene's candidate edges (THEORY.md §8).
    ...
    distance :func:`contact.geometry.observe` computes (§1), so the broad-phase metric
    ...
    for edge in scene.edges:
        ...
        # The support may be the implicit static "world" floor (THEORY.md §1), which the
...
# --------------------------------------------------------------------------------------
# The subset alphabet (THEORY.md §8: the hidden structure is *which edges are active*).
...
def _enumerate_subsets(num_edges: int) -> list[tuple[int, ...]]:
    ...
    the empty set. THEORY.md §8: this exact enumeration is the tractable choice for the
...
# --------------------------------------------------------------------------------------
# Joint emission (THEORY.md §8 & §4). For each frame t and each subset A we need
# log p(observations_t | active set = A). The per-edge detectors already gave us, per
# frame, P(edge active) = res.contact_posterior (a calibrated §4 posterior). Treating the
...
def _per_edge_log_evidence(
    ...
    per_edge_posterior:
        ...
        (P(edge active) per frame from its single-pair HMM, THEORY.md §4/§5).
    ...
    tuple[np.ndarray, np.ndarray]
        ...
        log-evidence rather than ``-inf`` (THEORY.md §4: evidence is soft).
    """
...
def _subset_log_emission(
    ...
    set (THEORY.md §4/§8) -- the per-subset sum. The construction now lives in
...
# --------------------------------------------------------------------------------------
# Temporal prior over the subset sequence (THEORY.md §5 lifted to §8). The active set
# is temporally coherent — the §8 structure persists. We put a Markov chain on the 2**E
...
def _subset_log_transition(
    ...
    THEORY.md §5), with the complementary mass split uniformly over the other subsets:
    ...
    log is finite (THEORY.md §5, the prior is soft).
    """
    if n_subsets <= 1:
    ...
    # Guard the floor so no entry is exactly zero (keeps the log finite, §5).
...
def _median_dt(t: np.ndarray) -> float:
    ...
    Mirrors :func:`contact.detector._median_dt` so the active-set chain is discretized on
    the same clock the per-edge HMMs use (THEORY.md §5).
    """
...
# --------------------------------------------------------------------------------------
# Optional global consistency factors (THEORY.md §8). Built in parallel in
...
def _consistency_factors(
    ...
    THEORY.md §8: these are *soft* global checks. If consistency hands back all-zeros
...
# --------------------------------------------------------------------------------------
# The graph detector (THEORY.md §8): the whole pipeline end to end.
# --------------------------------------------------------------------------------------


# --------------------------------------------------------------------------------------
# Subset emission factors: the (T, 2^E) grid as a SUM of SubsetFactors (THEORY.md §8)
# --------------------------------------------------------------------------------------
#
...
def detect_scene(
    ...
    """Infer the joint active-set posterior over a multi-body contact graph (THEORY.md §8).
    ...
    1. **Per-edge detection.** For every candidate edge run the single-pair estimator:
       ...
       body may itself be moving — the relative-frame payoff of §1 (a foot on a deck
    ...
    2. **Joint active-set inference.** With ``E`` edges, enumerate the ``2**E`` active
       ...
       need RJMCMC/particle methods, §8). For each frame and subset, the joint emission
       ...
       independent given the set, §4/§8), evidence drawn from each edge's
       ``contact_posterior`` (clipped off {0,1}). Optional **energy** and **balance**
       consistency factors (THEORY.md §8, the global dissipation/CoM checks) are added
       ...
       set temporally coherent (the §8 structure persists). Forward-backward over the
    ...
    config:
        ...
        and the rest configures every per-edge :class:`~contact.detector.ContactDetector`.
    edge_forces:
    ...
    # share a timebase, §8); fall back to a single-frame stub for an empty scene.
    ...
    # --- (1) Per-edge single-pair detection (THEORY.md §1 + §4-§7). --------------
    ...
    for j, edge in enumerate(edges):
        ...
        # The support may itself be moving (§1, a deck under a rider) OR the implicit
        # static "world" floor that the scene's `bodies` dict omits -- synthesize an
        # identity PoseTrajectory for the latter (THEORY.md §1, infinite-mass support).
        support = _resolve_support(scene, edge.support_body, moving)
        if support is None:
            raise KeyError(
    ...
    # The per-edge active/inactive log-evidence (THEORY.md §4/§8) is the common input to
    ...
    # Tractability fork (THEORY.md §8/§10, governed by config.inference.enumerate_max_edges):
    ...
    if E <= enumerate_max:
        ...
        # optional global consistency factors (energy / balance), each a no-op when off (§8).
        ...
        # self-transition (THEORY.md §5 lifted to the structure level, §8).
        ...
        # the first frame (mirrors the single-pair FREE-favouring init, §5).
        ...
        # smoother on the per-edge evidence (THEORY.md §8 "particle filter over the
        # discrete structure"; §10's large-E rung). It uses the SAME structure-level dwell
