#!/usr/bin/env python3
"""CLI: detect the joint contact-graph structure of a multi-body scene (THEORY.md s.8).
...
The headline check (THEORY.md s.1/s.8): in ``person_on_skateboard`` the ``person_board``
...
Emission velocity scales (THEORY.md s.4). The package defaults are tuned for typical
...
choice (s.9: keep the model physically parameterized, not overfit to one number), not a
...
def _scene_velocity_scale(scene: MultiBodyScene) -> float:
    ...
    ``"world"``, THEORY.md s.1) and take a high percentile of ``|v_tangent|``. The maximum
    ...
    free flight (THEORY.md s.3/s.4). Returns 0.0 if nothing can be measured.
    """
    scale = 0.0
...
def _config_for_scene(scene: MultiBodyScene) -> DetectorConfig:
    ...
    scale and the FREE velocity prior to cover the observed range (THEORY.md s.4: the
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        "scene (THEORY.md s.8) and score it against the simulator's withheld truth.",
    )
    parser.add_argument(
        "--scene",
    ...
    parser.add_argument(
        ...
        "(THEORY.md s.4/s.9). Default 5e-4.",
    )
    parser.add_argument(
    ...
    # --- the THEORY.md s.1/s.8 headline call-out for the skateboard scene. --------------
    if args.scene == "person_on_skateboard" and "person_board" in result.per_edge:
        ...
        print(
            ...
            f"(the THEORY.md s.1/s.8 relative-frame payoff)."
        )
